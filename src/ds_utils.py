...
def get_dataset(self):
    if self._ml_framework == 'pytorch':
        dataset, self._num_classes = get_pytorch_dataset(self._vars.input_dir, self._vars.dataset_format, "train", get_transform(True, self._vars), \
                                                self._vars.classes, self._vars.roi_info, self._vars.patch_info)
        self._dataset_val, _ = get_pytorch_dataset(self._vars.input_dir, self._vars.dataset_format, "val", get_transform(False, self._vars), \
                                            self._vars.classes, self._vars.roi_info, self._vars.patch_info)
