...
def create_base_model(name="ResNet50", weights="imagenet", height=None, width=None,
                      channel=3, include_top=False, pooling=None, alpha=1.0,
                      depth_multiplier=1, dropout=0.001):
    if not isinstance(height, int) or not isinstance(width, int) or not isinstance(channel, int):
        raise TypeError("'height', 'width' and 'channel' need to be of type 'int'")
        
    if channel <= 0:
        raise ValueError(f"'channel' must be greater of equal to 1 but given was {channel}")
