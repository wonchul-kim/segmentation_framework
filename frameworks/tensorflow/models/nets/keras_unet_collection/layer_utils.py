...
def CONV_output(X, n_labels, kernel_size=1, activation='Softmax', name='output'):
    '''
    Convolutional layer with output activation.
    
    CONV_output(X, n_labels, kernel_size=1, activation='Softmax', name='output')
    ...
    if activation:
        ...
        if activation == 'Sigmoid':
            ...
            activation_func = eval(activation)
            X = activation_func(name='output')(X)
